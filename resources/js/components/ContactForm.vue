...
<form v-if="showform" @submit.prevent="sendForm()">
    <div class="form-group mb-3">
        <input class="form-control" type="text" v-model.trim="name" placeholder="Name" :class="{'is-invalid':errors.name}">
        <p v-for="(error,index) in errors.name" :key="'name' + index" class="error">{{error}}</p>
    </div>
    <div class="form-group mb-3">
        <input class="form-control" type="email" v-model.trim="email" placeholder="Email" :class="{'is-invalid':errors.email}">
        <p v-for="(error,index) in errors.email" :key="'email' + index" class="error">{{error}}</p>
    </div>
    <div class="form-group mb-3">
        <textarea class="form-control" v-model.trim="message" placeholder="Message Here" :class="{'is-invalid' : errors.message}" cols="30" rows="10"></textarea>
